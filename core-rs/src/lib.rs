...
impl vs_document {
  ...
  fn add_dirty(&mut self, rect: Option<RectI>) {
    ...
    let Some(clamped) = rect.clamp_to_image(self.image_width_i32(), self.image_height_i32()) else {
      return;
    };
...
#[no_mangle]
pub unsafe extern "C" fn vs_add_blur_rect(
  doc: *mut c_void,
...
#[no_mangle]
pub unsafe extern "C" fn vs_move_annotation(
  doc: *mut c_void,
  index: u32,
  dx: i32,
  dy: i32,
...
#[no_mangle]
pub unsafe extern "C" fn vs_remove_annotation(
  doc: *mut c_void,
  index: u32,
) -> i32 {
  if doc.is_null() {
...
fn draw_command(
  ...
  match cmd {
    VsCommand::Rect(rect) => draw_rect(buf, image_width, image_height, stride, *rect, false, clip),
    VsCommand::FilledRect(rect) => draw_rect(buf, image_width, image_height, stride, *rect, true, clip),
    VsCommand::Ellipse(cmd) => draw_ellipse(buf, image_width, image_height, stride, *cmd, false, clip),
    VsCommand::FilledEllipse(cmd) => draw_ellipse(buf, image_width, image_height, stride, *cmd, true, clip),
    ...
    VsCommand::Text { text, cmd } => draw_text(buf, image_width, image_height, stride, text, *cmd, clip),
    VsCommand::Pixelate(cmd) => draw_pixelate(buf, image_width, image_height, stride, *cmd, clip),
...
fn transform_command_affine(
  ...
  match cmd {
    ...
    VsCommand::Text { cmd: text_cmd, .. } => {
      ...
      text_cmd.font_px = ((text_cmd.font_px as f32) * avg_scale).round().clamp(8.0, 256.0) as u32;
    },
    VsCommand::Pixelate(pixelate) => {
      let (x, y, width, height) = transform_rect_affine(
        pixelate.x,
...
fn resize_command(cmd: &mut VsCommand, from: RectI, to: RectI) -> bool {
  ...
  match cmd {
    ...
    VsCommand::Text { cmd: text_cmd, .. } => {
      ...
      let font_px = ((text_cmd.font_px as f32) * scale).round().clamp(8.0, 256.0) as u32;
    ...
    VsCommand::Pixelate(pixelate) => {
      ...
      if pixelate.x == to.x0 && pixelate.y == to.y0 && pixelate.width == width && pixelate.height == height {
        return false;
      }
      pixelate.x = to.x0;
      pixelate.y = to.y0;
...
mod tests {
  ...
  fn undo_and_redo_affect_rendered_pixels() {
    ...
    unsafe {
      ...
      assert_eq!(vs_render_full(doc, rendered.as_mut_ptr(), rendered.len()), 0);
