...
/// SQLite destructor marker for transient strings passed to `sqlite3_bind_text`.
private let SQLITE_TRANSIENT = unsafeBitCast(-1, to: sqlite3_destructor_type.self)

/// Sendable owner for the raw SQLite connection pointer used by the statistics actor.
...
final class StatisticsSQLiteLedger: @unchecked Sendable {
  ...
  func open(replayingInto session: StatsSession) throws {
    guard db == nil else {
      return
    }
  ...
  func performTransaction<T>(_ work: () throws -> T) throws -> T {
    guard let db else {
      throw StatisticsStoreError.sqlite("Statistics database is not open")
    }
  ...
  func insertEventIfNeeded(_ event: StatsEvent, sourceType: String) throws -> Bool {
    guard let db else {
      throw StatisticsStoreError.sqlite("Statistics database is not open")
    }
  ...
  func firstOccurredAt(sourceType: String) throws -> Date? {
    guard let db else {
      throw StatisticsStoreError.sqlite("Statistics database is not open")
    }
  ...
  private func replayLedger(into session: StatsSession) throws {
    guard let db else {
      throw StatisticsStoreError.sqlite("Statistics database is not open")
    }
  ...
  private func eventType(from sourceType: String) throws -> StatsEventType {
    switch sourceType {
