...
extension AnnotationCanvasView {
  ...
  func imageRectFromViewRect(_ viewRect: CGRect) -> CGRect? {
    ...
    return CanvasGeometry.viewRectToImageRect(
      viewRect: viewRect,
  ...
  func viewRectFromImageRect(_ imageRect: CGRect) -> CGRect? {
    ...
    return CanvasGeometry.imageRectToViewRect(
      imageRect: imageRect,
      destinationRect: destination,
      imageSize: CGSize(width: image.width, height: image.height)
    )
  ...
  func viewDeltaFromImageDelta(_ delta: CGPoint) -> CGPoint? {
    ...
    return CanvasGeometry.imageDeltaToViewDelta(
      delta,
      destinationRect: destination,
      imageSize: CGSize(width: image.width, height: image.height)
    )
  ...
  func resizedAnnotationBounds(
    ...
    return CanvasGeometry.resizeRect(
      start: start,
      bounds: imageRect,
      edge: resizeEdge(for: handle),
      delta: delta,
      minWidth: 6,
      minHeight: 6
    )?.integral
  ...
  private func clampedPanOffset(for image: CGImage, zoomScale: CGFloat, candidate: CGPoint) -> CGPoint {
    ...
    return CanvasGeometry.clampPanOffset(
  ...
  private func resizeEdge(for handle: AnnotationResizeHandle) -> ResizeEdge {
    switch handle {
    case .topLeft:
      return .topLeft
    case .topRight:
