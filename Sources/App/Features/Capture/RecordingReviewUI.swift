...
/// Floating heads-up display shown while a recording is active.
@MainActor
...
/// User decision from the post-recording review panel.
...
/// Export options that affect codec, quality, scale, and bitrate for video output.
...
/// Trim and audio state passed to the exporter for one save action.
...
struct ProExportRequirement: Equatable {
  ...
  static func evaluate(
    ...
    var reasons = project.videoProject.proRequirement(target: target, options: options) ?? []
    if !includesAudio {
