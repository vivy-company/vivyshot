...
struct PaywallView: View {
  ...
  @State private var selectedPlan: PlanKind = .lifetime
  ...
  private var availablePlans: [PlanKind] {
    if storeManager.hasSupporterBadge {
      return []
    }
  ...
  private var defaultPlan: PlanKind {
  ...
  private func product(for plan: PlanKind) -> Product? {
    switch plan {
    case .lifetime:
  ...
  private func isOwned(_ plan: PlanKind) -> Bool {
    switch plan {
    case .lifetime:
...
private enum PlanKind: String, CaseIterable, Identifiable {
  case lifetime
  case supporter
