...
extension RegionSelectionView {
  ...
  func startStitchRecording() {
    ...
    guard let newStitchSession = StitchSession(),
          newStitchSession.setBaseImage(baseImage, baseSegmentCount: stitchSegmentCount)
    else {
      NSSound.beep()
      return
    }

    stitchSession = newStitchSession
  ...
  func resetStitch() {
    ...
    guard let preStitchImage,
          ...
          let restoredSession = AnnotationSession(image: preStitchImage)
    else {
      NSSound.beep()
      return
    }
  ...
  func cropSegment(
    ...
    return ScreenshotImage.crop(image, imageRect: integral) ?? image.cropping(to: integral)
  }
}
