...
final class VivyShotTests: XCTestCase {
  ...
  func testCaptureCapabilitiesComeFromRustBackend() {
    var capabilities = vs_capture_capabilities()
    XCTAssertEqual(vs_capture_copy_capabilities(&capabilities), Int32(VS_CAPTURE_STATUS_OK))
    XCTAssertTrue(capabilities.region_recording)
    XCTAssertTrue(capabilities.display_recording)
    XCTAssertTrue(capabilities.direct_to_file_recording)
    XCTAssertTrue(capabilities.h264)
    XCTAssertTrue(capabilities.screenshot_capture)
  }

  func testCaptureScreenshotClientBuildsCGImageFromOwnedBGRA() throws {
    let pixels: [UInt8] = [
      10, 20, 30, 255,
      40, 50, 60, 255,
      70, 80, 90, 255,
    ...
    let image = try pixels.withUnsafeBufferPointer { buffer in
      try CaptureScreenshotClient.makeImage(
        from: vs_capture_captured_image(
          width: 2,
          height: 2,
          bytes_per_row: 8,
          pixel_format: UInt8(VS_CAPTURE_PIXEL_FORMAT_BGRA8_PREMULTIPLIED_FIRST),
          data: buffer.baseAddress,
          data_len: UInt32(buffer.count)
        )
      )
    }
  ...
  func testCaptureScreenshotClientRejectsInvalidBGRA() {
    let image = vs_capture_captured_image(
      width: 2,
      height: 2,
      bytes_per_row: 4,
      pixel_format: UInt8(VS_CAPTURE_PIXEL_FORMAT_BGRA8_PREMULTIPLIED_FIRST),
    ...
    XCTAssertThrowsError(try CaptureScreenshotClient.makeImage(from: image))
  }

  @MainActor
  func testRecordingOverlaySmokeHarnessExposesCapturableMovableOverlays() {
    let result = RecordingOverlaySmokeHarness.inspectAndMove()
    XCTAssertNotNil(result.capturedWindowID)
    XCTAssertTrue(result.webcamCallbackFired)
    XCTAssertTrue(result.keystrokeCallbackFired)
  }

  @MainActor
  func testLiveVideoRecordingSmokeWhenEnabled() async throws {
    let markerURL = URL(fileURLWithPath: "/tmp/vivyshot-live-video-smoke")
    guard ProcessInfo.processInfo.environment["VIVYSHOT_LIVE_VIDEO_SMOKE"] == "1"
      || FileManager.default.fileExists(atPath: markerURL.path)
    else {
      throw XCTSkip("Set VIVYSHOT_LIVE_VIDEO_SMOKE=1 or create /tmp/vivyshot-live-video-smoke to run live ScreenCaptureKit recording smoke.")
    }

    for encoder in [VideoRecordingEncoderOption.standardH264, .cpuH264] {
      ...
      XCTAssertGreaterThan(result.videoSize.width, 0)
      XCTAssertGreaterThan(result.videoSize.height, 0)
      XCTAssertTrue(result.containsOverlayColor, "Expected \(encoder.title) smoke recording to include red overlay window.")
      XCTAssertTrue(
        result.excludesCurrentProcessWindow,
        "Expected \(encoder.title) smoke recording to exclude non-whitelisted current-process windows."
      )
    }
  }
