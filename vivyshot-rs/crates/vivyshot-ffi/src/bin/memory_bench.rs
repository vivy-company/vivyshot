...
use vivyshot_core::{
    ...
    vs_arrow_command, vs_blur_rect_command, vs_create_document_from_bgra, vs_destroy_document,
    vs_dirty_rect, vs_line_command, vs_pixelate_rect_command, vs_rect_command, vs_redo,
    vs_render_dirty, vs_render_full, vs_text_command, vs_undo,
};
...
fn run_session(index: usize) -> u64 {
    ...
    unsafe {
        ...
        assert_eq!(vs_add_text(doc, label.as_ptr(), label.len(), text_cmd), 0);
        render_dirty(doc, &mut out);
