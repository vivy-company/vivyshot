...
extension RegionSelectionView {
  ...
  func layoutVideoOverlayPlacementViews(selection: CGRect?) {
    ...
    if settings.showWebcam {
      ...
      webcamPlacementView.frame = resolvedWebcamOverlayFrame(settings.webcamOverlayNormalizedFrame, in: selection)
    ...
    if settings.highlightKeystrokes {
      ...
      keystrokePlacementView.frame = resolvedOverlayFrame(settings.keystrokeOverlayNormalizedFrame, in: selection)
