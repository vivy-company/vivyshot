...
/// Common lifecycle contract for hardware and software recording implementations.
@MainActor
...
final class RecordingCoordinator {
  private let microphoneFeatureEnabled = true
  private let webcamFeatureEnabled = true
  private let keystrokesFeatureEnabled = true
  ...
  func startRecording(
    ...
    Task { [weak self] in
      ...
      do {
        let initialOverlayState = overlayState ?? RecordingOverlayState.from(settings: settings)
        ...
        webcamPlacementChanges = [
          OverlayPlacementChange(timestampSeconds: 0, normalizedFrame: initialWebcamFrame)
        ]
        keystrokePlacementChanges = [
          OverlayPlacementChange(timestampSeconds: 0, normalizedFrame: initialOverlayState.keystrokeFrame)
        ]
        ...
        if webcamEnabled {
          ...
          let webcamRecorder = try WebcamRecorder(
            ...
            preferredDeviceID: settings.webcamDeviceID
          )
        ...
        let recorder: any RegionRecordingSession = ScreenRegionAssetWriterRecorder(
  ...
  private func setWebcamDeviceIDForActiveRecording(_ deviceID: String) async {
    ...
    do {
      ...
      onError?("Failed to update webcam source: \(error.localizedDescription)")
    }
  }
  ...
  private func recordRecordingStatisticsIfNeeded(inputURL: URL, durationSeconds: Double) {
    ...
    Task {
      await StatisticsStore.shared.recordRecordingCompleted(
  ...
  private func quickSaveVideo(
    ...
    Task {
      do {
        ...
        let shouldUseCustomCompositor = project.videoProject.hasMouseClickOverlays
  ...
  private func makeVideoProject(
    ...
    _ = videoProject.setKeystrokeOverlay(
      ...
      style: settings.keystrokeOverlayStyle,
  ...
  private var effectiveCaptureMicrophoneEnabled: Bool {
    microphoneFeatureEnabled && settings.recordMicrophone
  }

  private var effectiveShowWebcamEnabled: Bool {
    webcamFeatureEnabled && settings.showWebcam
  }

  private var effectiveHighlightKeystrokesEnabled: Bool {
    keystrokesFeatureEnabled && settings.highlightKeystrokes
  }
}

/// Immutable configuration passed to a concrete screen recording backend.
...
struct RecordingOverlayState {
  ...
  private static let fallbackFrame = CGRect(x: 0, y: 0, width: 0.2, height: 0.2)
  private static let minimumDimension: CGFloat = 0.04
  ...
  static func normalizedFrame(_ frame: CGRect) -> CGRect {
    let source = frame.isNull || frame.isEmpty ? Self.fallbackFrame : frame.standardized
    let width = max(Self.minimumDimension, min(1, source.width))
    let height = max(Self.minimumDimension, min(1, source.height))
...
private final class RecordingAssetWriter: @unchecked Sendable {
