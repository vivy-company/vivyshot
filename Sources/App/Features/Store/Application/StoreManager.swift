...
/// StoreKit facade that loads products, tracks entitlements, and exposes paid access.
@MainActor
final class StoreManager: ObservableObject {
  ...
  private(set) var storeKitTasksStarted = false
  ...
  var lifetimeProduct: Product? {
    products.first { $0.id == StoreProducts.lifetime }
  }

  var supporterProduct: Product? {
    products.first { $0.id == StoreProducts.supporter }
  }

  init(localizer: AppLocalizer, automaticallyStartsStoreKit: Bool = true) {
    self.localizer = localizer
    if automaticallyStartsStoreKit {
      startStoreKitTasksIfNeeded()
    }
  }

  func startStoreKitTasksIfNeeded() {
    guard !storeKitTasksStarted else {
      return
    }
    storeKitTasksStarted = true
  ...
  func loadProducts() async {
    do {
      let loadedProducts = try await Product.products(for: StoreProducts.allProductIDs)
      products = loadedProducts.sorted { lhs, rhs in
        StoreProducts.allProductIDs.firstIndex(of: lhs.id) ?? .max <
          StoreProducts.allProductIDs.firstIndex(of: rhs.id) ?? .max
      }
  ...
  func purchase(_ product: Product) async {
    guard !isProductAlreadyActive(product.id) else {
      purchaseState = .failed(String(localized: "Already Owned", bundle: localizer.bundle))
      return
    }
  ...
  private func isProductAlreadyActive(_ productID: String) -> Bool {
    switch productID {
    case StoreProducts.lifetime:
      return hasLifetimeUnlock || hasSupporterBadge
    case StoreProducts.supporter:
